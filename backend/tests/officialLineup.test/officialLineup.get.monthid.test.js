...
describe("OfficialLineup Endpoints", () => {
    ...
    describe("GET /api/officialLineup/:monthId", () => {
        let token;
        let monthId;
        let players;
        let officialLineup;
        ...
        // Test création du Top 5 officiel du mois
        it("Retourne le OficielLineup du mois par l'id du mois", async () => {
            const res = await request(app)
                .get(`/api/officialLineup/${monthId}`)
        ...
        // Test si mois n'existe pas
        it("Retourne 404 si le mois n'existe pas", async () => {
            const res = await request(app)
                .get("/api/officialLineup/99999")
                .set("Authorization", `Bearer ${token}`);
        ...
        it("Retourne 404 si aucun OfficialLineup n'existe pour ce mois", async () => {
            // Création d’un nouveau mois sans lineup
            ...
            const res = await request(app)
                .get(`/api/officialLineup/${newMonth.id}`)
                .set("Authorization", `Bearer ${token}`);
