...
describe("Lineup Endpoints", () => {
    ...
    describe("GET /api/lineups/:monthId", () => {
        let token;
        let monthId;
        let players;
        ...
        it("Récupère les Top 5 du mois courant", async () => {
            const res = await request(app)
                .get(`/api/lineups/${monthId}`)
                .set("Authorization", `Bearer ${token}`);

            expect(res.statusCode).toBe(200);
            expect(res.body.success).toBe(true);

            const lineupsReturned = res.body.data;

            expect(Array.isArray(lineupsReturned)).toBe(true);
            expect(lineupsReturned.length).toBeGreaterThan(0);

            lineupsReturned.forEach(lineup => {
                expect(lineup.MonthlyPlayers.length).toBe(5);

                lineup.MonthlyPlayers.forEach(p => {
